    ...
    <DashboardLayout merchant={merchant}>
      <div className="space-y-6">
          ...
          {referralCode ? (
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                <div className="flex-1 bg-slate-50 border-2 border-dashed border-pink-300 rounded-xl px-6 py-4 text-center">
                  <span className="text-3xl font-bold tracking-widest text-violet-700">
                    {referralCode}
                  </span>
                </div>
                <Button
                  onClick={copyCode}
                  variant="outline"
                  className="h-14 px-4 border-pink-300 hover:bg-pink-50"
                >
                  {copied ? (
        ...
        <Card className="p-6 bg-pink-50 border-pink-200">
          <h3 className="text-lg font-semibold text-violet-900 mb-3">Comment ça marche ?</h3>
          <div className="space-y-3">
            <div className="flex items-start gap-3">
              <span className="w-6 h-6 bg-pink-600 text-white text-xs font-bold rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">1</span>
              <p className="text-sm text-violet-800">Partagez votre code de parrainage avec un autre commerce</p>
            </div>
            <div className="flex items-start gap-3">
              <span className="w-6 h-6 bg-pink-600 text-white text-xs font-bold rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">2</span>
              <p className="text-sm text-violet-800">Votre filleul s&apos;inscrit avec votre code</p>
            </div>
            <div className="flex items-start gap-3">
              <span className="w-6 h-6 bg-pink-600 text-white text-xs font-bold rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">3</span>
              <p className="text-sm text-violet-800">Vous recevez tous les deux <strong>50 crédits de campagne</strong> !</p>
            </div>
          </div>
        </Card>
                ...
                <tbody>
                  {referrals.map((ref) => (
                    <tr key={ref.id} className="border-b border-slate-100 last:border-0">
                      ...
                      <td className="py-3 px-2 text-right font-semibold text-pink-600">
                        +{ref.credit_amount}
                      </td>
                    </tr>
                  ))}
